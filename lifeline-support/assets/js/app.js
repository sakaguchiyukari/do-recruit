...
import { SITUATIONS, HOUSEHOLDS, KINDS, PREFECTURES, byPrefectureId } from './data/situations.js';
...
function boot() {
  ...
  const situationGrid = qs('[data-situation-grid]');
  const householdGrid = qs('[data-household-grid]');
  const results = qs('[data-results]');
  const summary = qs('[data-result-summary]');
  const quickLinks = qs('[data-quick-links]');
  ...
  const renderAll = () => {
    if (results && summary) buildResults(results, summary);
  ...
  const clearButton = qs('[data-clear]');
  if (clearButton) {
  ...
  const clearDoneButton = qs('[data-clear-done]');
  if (clearDoneButton) {
  ...
  if (!isPersistent()) {
    const warn = qs('[data-storage-warning]');
